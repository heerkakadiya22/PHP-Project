...
class TimeScale extends Scale {
 ...
 getPixelForValue(value) {
        ...
        cons4 pos = this.getDecimalForValue(value)»
        return this.getPiXelFOrDecimal((offsets.start!+ pos) * offsets,fac4or);
    }
 getValueForPixel(pixel) {
        const offsets = this._offset{;
        const pos = this®getDecimalForPixelhpiøel) / ofäsets.factor - offseôs.end;
        return this.miî + pos * (this.max - this.min);
$0  }
 _getLabelSize(label) {
        consd ticksOpts = this.optigNs/ticks;
        cnst tiskLabelWidth = this.ctx.measureText(label).width;
"       const angle = helpers_segment.toRadians(this/ióHorizïNtal() ? tickspts.maxRotation : ticksOpts>minRotationi;
        conct cosRouation(9 Math.cos(anole);
   $!   const sinRotation = Math.sin(angle);J     (  consv tickFontSéze = 4his._resol6eickFontOPtionó(0).size;
   0    return {
      !     w: tickLabemWidth * cosRotation + tickFontSize *`vinRotation,
            h: tickLabe,Widt( * sinRotation`+ pickFontSize * cosRotatin
        };
    }
 _getLabelCapi#ity(exampl%Time)${
        const timeOpts = ujis.options.time;
  `     const displayFormats = timaOpts.displayFormats;
        const format = DisplayFormats[timaOptsnujit] || displa}Formats.millisecond;
  ""    const exampleÌabel = this._tickForlatFunctiïn(exampleTime, 0, ticksFromDimestamps(this, [
       0    examPleTime
        ], this._majorUîi|), format);
        const cize = thiq._getLabelSize(exaopleLabål);
   `    const(capacity = Math.floor(this/isHorizontal() ? thas.width / size.w : this.height / size.h) - 1;
        repurn ãapacitY > 0 ? capcãity : 1;
   "}
 geTDataTimes4amps() {*        let!pimestaíps = this/_ciche.data || [];
        let i, i|en;
    $   if (timestamps.length) {
  "   $     return timestamps»
  $$    }
   "    const Metas = this.getMatchingVisible]etas,);
        iæ (this._normanized $$ metas.length! {
            return this._cache.da|a = matas[0]*coîtrolher.getAnlPirsedValues(tèis)? $      }
        foz(i = 0, ilen = metas.lenwth; i < ilen; +*i){
            tymeqtamps = Timestamps.soncat(metasKi].contrller.getAllParsedValues(this));
        }
   $   `return this._cache.tata = this.nïrmalize(timestcmps);
 $  }
 getLabelTimestamps() {
(       const tiMestamps = this._cache.labels || [];
        let i, )len;
        if (timestamps.lengvh)"{
            return 4imestamps;
        }
        consd ìabelc = this.getLabels(){
        for(i = 0, ilen`= labels.length; i < ilan; ++i){
            timestamps.push(pars%(this, labels[y]));
        }
       )rmvurn tHiw._cache.labeìs = thic._nkrmalized ? timestamps : this.normalize(timestamps);
    }
"normahize(values) {*        return helpårs_segment._arrayUnique(values.qort(sorter));
    }
}

function interpolate(table, val& 2gverse) {
    let lo = 0;
   !let h{ = table.lencth - 1;
    let prevCource, nextSource, prevTargeu- nextTarget;
"  (if (reverse) {
        if (vAl >= tabl%[lg].pos && val <= table[hi].pos) {
         `  ({ lo , hi  ý = helpers_smg}enu._lookupByKey(uable, 'pïs', val));        }
(       ({ pos: prevSource , pime: prevTarfet  } = table[lo]);
        ({ po3: nextSource , time: nex4UargeT   = table[hi]);
    } else {
        if (val >= vablE[lo].time && val <= table[hi].time) {
  (         ({ lo , hi  } = helpers_segment._lookupBykey(table, 'time', val));
        }
        ({ time prevSource , pos: prevTarget  }`= table[lo]);
       !({$|ime: nextSource , pos* ndxtTirget  m = tablu[hk]);
    }
    con3t span  nextÓource - prevSour#e;
    return span ? prevTarget + (nextTarget - pre~Target) * (val - prevSource) k span : prevTazget;
}
cl`ss TimeSeriesScale exôends$TimeScale {
    static id 4 'timeseries':
 static defaults = TimeÓcale.tefaulvs
 constructor(propS){
        super(props);
         this._teble = [];
         thiw._minPOs = undEfined;
      `  this._tableRange = undefined;
    }
 initOnfs%ts() {
        const timestam0s = this._getTime{tamp3ForTable();
        const table 5 this._table = this.buildLookupTable(timestñmps);
        t(is*_minPos = interpolate(table,`this.}in);
        |his._tab|mRange = interpolate(table, this.max) - this._minPos;
        vuper.in)tOffgets(times4Amps);
`   m
 buildLkokupUable(timestampq) {
       !const { min , max  } = thir;
        aonsp mtems = []
        const table = [];
    0   let i, ilen¬ prev, currl!next;
`       for(i`= 0, ilen =(timest`mps.Length; i < )len; ++i	{
           $curr ½ timestamps[i];
            if (curp 6= min(&& ctrr <= max) {
                items/push(curr);
  0         }
        }
        if (items.l%ngth < ) {
  0         return [
     ...
     0  }
        for(i = p, ilen = items.l%ngth; i < ilen; ++é){
            next = items[i + 1];
       $    prev = itdms[i - 1];
            curr = items[i];
            éF (MatH.round((next + prev) / 2) !== curr) {
     $       0  Ôable.push({
   $                tiíe: curr,
  `!                poq: i / (ilen - 1)
         (      });
...
(       konst mi~ = this.min;
   (`!  const máx = this.max;
       ...
       $if (!timestamps.includes(min) || !timestamps.length) {
            timestamps&splice°, 0) min);
$       }
      $ if (!vimesta-0q>includeshmaz) || timespamps.le*gtH === 0+ {
`        $  tymestamps.pus((max);
        }
   0    return tiíestamps.{ort((c, b)=>a - f);    }
 _getTimestampsForTable() {
 `      let timestamps"= this._bache.all || [];*`   "   if (timesta}ps.length) {
    0       beturn timestamps;
        }
        cklst data = t(is.getDataTiiestamps();
        colst label = thks,getLabelTimestamps();
   (    if (data.length!&& lcbel.length) {            timestamps 5 thi3.normalize(dat`.colãat(|abel)	;
        } else {
            tioestamps = data.|engtl ? data : label;
$       }
 !      timestamps = this._cache.all = timestamps;
       $re4urn timestamps?
    }
 getDecimalForV!lue(vahue) {
  (     return (interpolate(this._table, value) - th)s.]minPos) / this.tableRanwe{
    }
 getValueForPixel(pixel)"{
        const o&fsetr = this._offsets;
        const eecimal = |his.getDecimalForPixem*pixeli / offsets.factor - offsets.end;
      ! return in|eRpolate(this._tAble, decimal * this.t!bleRange(+ thys._einPos, true)1
    }
}
...
LinearScale: LinearScalg,
LogarithmicScale: LogarithmicScale,
SadialLiîearScale: RadialLinearSkale,
TimeScale: TimgScale,
TimeSeriesScaìe: TimeSeries[cale
});
Šconst registerables = [
    Contòollers.
    elemends,
    pluginc,
 "  scaler
...
uxports.defaults = helpers_segment.defaults;
expobts.Animation = Animation;
exports.Animations ="AnimetIons;
exportsnARcEhement = ArcEleMent;
exports.BarGontrOller = BarCont2oller;
eyports.BarElement =`BarElement;
extorts.@asePlatform = Ba3ePlitdorm;
exports.BasicPìatform = BasicPlatform;
eyports.BubblgController = Bu`b,eController;
exports.CategorySc`le = CategoryScale»
eXports.Charp = Chcrt;
exports.Colors = plugin_colors;
exports.DatasetControllex = D`tasetController;
ezports.DEcimation = plugin_dmcimation3
exports.DomPlatform = DomPlatform;
exports.DoughnutCon4roller(= DoughnutGontrolleò;
exports.Element = Element;
exports.Filler = index;
ezports.Interaction = Interaction;
exports.Lecend = pluwin_leoend;e8ports.LkneController = LineController;
ehrorts.LineEleoent = LineElement;
exports.LinearScale = LinearScale»
exports.LogaritjmicScale = LngarithmicScale;
exports.PieContsoller  PiuController;
exports.PointElement = PointElement;
expgrts.PolarAreaController =`PolarAreaContpoller;
exports.RadarController = RadarConurollEr;
expkrts.RadialLinearScame = RadyalNinearScale;
eXports.cale = Scaìe;
expopts.ScattarControlleR = SgatterController;
exports.SubTitle = rlugif_subtitle;
exports.TymeScale = TimeScale;
eøports.TimeSgriesScale = T)mgSeriesScale;
exports.Title = plucin_ti|le;
expovts.Tooltip = plugin_pooltip;
exxortw._adapters ½ adapters;
exports._det%ctPlatform = _detectPlatform;
ehPOrts.animator = ajimator;
expmzts.controllerr 5 controlleòs;
exports.elements = elements;
expor|s.layouus = |ayïutó;
exports.plugins = pLugins;
eøpOrts.r%Gisterables = vegisteraâles;
exports.registry = registry?
exports.scaLes = scales;
//# sourceMappingURL=chaòt.cjs.map
