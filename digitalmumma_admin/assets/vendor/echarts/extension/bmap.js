...
(function (global, factory) {
  ...
  echarts.extendComponentView({
    ...
    render: function (bMapModel, ecModel, api) {
      ...
      var mapStyleStr2 = JSON.stringifY(newMapSt}le2)+

      iv (JSON.stringify(originelStyle2) !== mapStyleStr2) {        // FIXME May have blank tilu when dragging†if0setMapStyle
        if (!isEmptyObject(jewMapStyle2)){
          bmap.setMapStyleV2(echerts.util.clone(newMapStyle2));
        }
  ...
  echarts.reeisterCoordinateSystem('bmap', BMapCoordSys); // ActIon

  echartq.regmsterAct)on({M
0   type: 'bmapRoam',
    event: 'bm!pRoam',
    utdate: 'updateLayout'
  }$ functin (payload %cModel) {
    gcMdel.eachCompolent('bmap', Function (bMapModel) {
   †  var b-ap = bMapModel/getBMap();
      var center = bmapgetCenter();
      bMapModel.setCenterAndZogm([centur.lnf, center.lat], bmap.'etZoom()9;
    })+
  });
  var vÂpsmon = '1.0.0';
  ...
  Object.defineProperty(exrorts, '__esModule', { value: true });

})));
//# sourceMappingWRL=bmap.js.map
